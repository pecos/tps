...
void OutletBC::updateMean_gpu(ParGridFunction *Up,
                              Vector &localMeanUp,
...
void OutletBC::integrationBC( Vector& y, 
                              const Vector &x, 
                              ...
                              const int& maxDofs)
{
  integrateOutlets_gpu( outletType,
                        inputState,
                        dt,
                        y, // output
                        x, 
...
void OutletBC::subsonicNonRefMassFlow(Vector &normal,
                                      Vector &stateIn, 
                                      DenseMatrix &gradState,
                                      Vector &bdrFlux )
{
...
void OutletBC::subsonicNonRefPWMassFlow(Vector &normal,
                                        Vector &stateIn, 
                                        DenseMatrix &gradState,
                                        Vector &bdrFlux )
{
      ...
      for(int q=0;q<Q;q++) // loop over int. points
      {
        ...
        if(i<num_equation)
        {
          ...
          for(int k=0;k<elDof;k++)
          {
            ...
            for(int d=0;d<dim;d++) gradUp[i+d*num_equation] += 
                        gradUpi[k+i*elDof+d*num_equation*elDof]*shape[k];
          }
        }
        MFEM_SYNC_THREAD;
        ...
        switch(type)
        {
          case OutletType::SUB_P:
            if(i<num_equation) computeSubPressure(i,&u1[0],&u2[0],&nor[0],
                                                 d_inputState[0],gamma,dim,num_equation );
            break;
