...
void ThermoChem::setup(double dt) {
  ...
  if (numWalls > 0) {
    ConstantCoefficient t_bc_coef0(wallBCs[0]);
  }
  if (numWalls > 1) {
    ConstantCoefficient t_bc_coef1(wallBCs[1]);
  }
  if (numWalls > 2) {
    ConstantCoefficient t_bc_coef2(wallBCs[2]);
  }
  if (numWalls > 3) {
    ConstantCoefficient t_bc_coef2(wallBCs[3]);
  }
      ...
      if (config_->wallPatchType[i].second == WallType::VISC_ISOTH) {
        if (iFace == config_->wallPatchType[i].first) {
          ...
          if (config_->useWallFunction == true) {
            ...
            if (i == 0) {
              buffer_tbc = new ConstantCoefficient(t_bc_coef0);
            }
            if (i == 1) {
              buffer_tbc = new ConstantCoefficient(t_bc_coef1);
            }
            if (i == 2) {
              buffer_tbc = new ConstantCoefficient(t_bc_coef2);
            }
            if (i == 3) {
              buffer_tbc = new ConstantCoefficient(t_bc_coef3);
            }
