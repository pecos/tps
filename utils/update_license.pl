...
# 
# Originally: August 2010 (karl@oden.utexas.edu)
# --------------------------------------------------------------------
...
# Multiple language support; in certain instances, we have multiple languages
# within the same project (eg. libgrvy and libmasa which provide interfaces to
# both C and Fortran). We would like to trigger the license replacement off of
# a single file only and embed desired comment syntax here if desired.

my $comment = "//";
if ($opt_c2sh_comment) {
    $comment = "#";
} elsif ($opt_c2f_comment) {
    $comment = "!!";
}

# Update license with comment characters
my @license_tmp=();
foreach $line (@license_text) {
    if($line eq "\n") {
        push(@license_tmp,$comment.$line)
    } else {
        push(@license_tmp,$comment." ".$line);
    }
}
...
while (@ARGV) 
{
	...
	if ( compare($infile,$tmpfile) != 0 )  {
	    print "[license_tool]: updated license in file $infile\n";
