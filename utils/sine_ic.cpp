...
int main (int argc, char *argv[])
{
  ...
  std::string basepath("utils/sine_initial_condition");
  ...
  for (int i = 0; i < NDof; i++) {
    double phase = pi * (coordinates[i + 0 * NDof] - offset) / scale;
    double sineFactor = 0.0;
    if (phase > 0.5 * pi) {
      sineFactor = 1.0;
    } else if (phase < - 0.5 * pi) {
      sineFactor = 0.0;
    } else {
      sineFactor = 0.5 + 0.5 * sin(phase);
    }
    ...
    for (int eq = 0; eq < num_equation; eq++) {
      dataU[i + eq * NDof] = sol1[eq] * (1.0 - sineFactor) + sol2[eq] * sineFactor;
    }
  }
  ...
  for (int i = 0; i < NDof; i++) {
    double phase = pi * (coordinates[i + 0 * NDof] - distance - offset) / scale;
    double sineFactor = 0.0;
    if (phase > 0.5 * pi) {
      sineFactor = 1.0;
    } else if (phase < - 0.5 * pi) {
